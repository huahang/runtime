...
2. **V2Ray (v5.52.0)**: Clones the pinned tag and runs `release/user-package.sh` with `CGO_ENABLED=0` and explicit `amd64`/`arm64` architecture selection.
...
- V2Ray: `v5.52.0` (from `v2fly/v2ray-core`)
